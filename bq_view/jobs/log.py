...
class LogSchemaJob(BaseABC, metaclass=abc.ABCMeta):

    def sync(self, source_table, schema, is_recreated):
        destination_dataset = f"{schema.upper()}_MIRROR_LOG"
        ...
        # is_recreated mean mirror has been recreated so log need to re-create(encrypted cols check)
        # Check all unencrypted cols with stage and log table
        if is_recreated or self.has_drifted(source_table, created_dataset,destination_table_id):
            ...
            other_cols = [ #'schema_key', 'uuid', 'read_method', 'object', 'sort_keys',
